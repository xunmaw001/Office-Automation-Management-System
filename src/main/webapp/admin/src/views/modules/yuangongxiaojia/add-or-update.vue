...
export default {
  ...
  methods: {
	...
	this.ruleForm.qingjiayuanyin = this.ruleForm.qingjiayuanyin.replace(reg,'../../../ssm203t6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yuangongxiaojia/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yuangongxiaojiaCrossAddOrUpdateFlag = false;
                  this.parent.search();
