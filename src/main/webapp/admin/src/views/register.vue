  ...
  <div>
    <div class="container">
      <div class="login-form" style="backgroundColor:rgba(255, 255, 255, 0.26);borderRadius:10px">
        <h1 class="h1" style="color:rgba(255, 255, 255, 1);fontSize:28px;">办公自动化管理系统注册</h1>
		<el-form ref="rgsForm" class="rgs-form" :model="rgsForm" label-width="120px">
			...
			<el-form-item label="密码" class="input" v-if="tableName=='bumen'">
			  <el-input v-model="ruleForm.mima" autocomplete="off" placeholder="密码" type="password"#elsetype="text" />
			</el-form-item>
			<!-- <div v-if="tableName=='bumen'" class="input-group">
			   <div class="label">部门姓名</div>
			   <div class="input-container">
			     <input v-model="ruleForm.bumenxingming" class="input" type="text" placeholder="部门姓名">
			   </div>
			 </div> -->
			<el-form-item label="部门姓名" class="input" v-if="tableName=='bumen'">
			  <el-input v-model="ruleForm.bumenxingming" autocomplete="off" placeholder="部门姓名"  />
			</el-form-item>
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      if((!this.ruleForm.bumenzhanghao) && `bumen` == this.tableName){
        this.$message.error(`部门账号不能为空`);
        return
      }
      if((!this.ruleForm.mima) && `bumen` == this.tableName){
        this.$message.error(`密码不能为空`);
        return
      }
      if((!this.ruleForm.bumenxingming) && `bumen` == this.tableName){
        this.$message.error(`部门姓名不能为空`);
        return
      }
      this.$http({
...
<style lang="scss" scoped>
 ...
 //  }
	...
	.container {
		background-image: url("http://codegen.caihongy.cn/20210411/67efe91d931f447b810789ec89f0f3c4.jpg");
		height: 100vh;
